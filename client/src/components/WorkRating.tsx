...
interface WorkRatingProps {
  workId: number;
}

export default function WorkRating({ workId }: WorkRatingProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg">作品评分</CardTitle>
          <CardDescription>用户对此作品的评价</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      {isAuthenticated && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {myRating > 0 ? '您已对此作品评分，可以修改评分' : '为此作品打分'}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
