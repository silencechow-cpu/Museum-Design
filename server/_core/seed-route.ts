...
export async function runMigrations(connection: mysql.Connection, logs: string[]) {
  const log = (msg: string) => { console.log(msg); logs.push(msg); };

  // Find migrations directory relative to this file
  // In production: dist/index.js -> look for drizzle/ relative to project root
  const possiblePaths = [
  ...
  if (!migrationsDir) {
    log("⚠️  Migrations directory not found, skipping migrations");
    return;
  }

  log(`📂 Found migrations at: ${migrationsDir}`);
      ...
      if (stmt) {
        try {
          ...
          if (err.code === "ER_TABLE_EXISTS_ERROR" || err.code === "ER_DUP_FIELDNAME" || err.code === "ER_DUP_KEYNAME") {
            log(`    ⚠️  Skipped (already exists): ${err.message.substring(0, 60)}`);
          } else {
            throw err;
          }
        }
...
export function registerSeedRoute(app: Router) {
  // Migration endpoint - run DB migrations
  app.post("/api/migrate", async (req, res) => {
    ...
    try {
      ...
      await runMigrations(connection, logs);
    ...
    try {
      ...
      const museums = [
        ...
        },
        {
          ...
          name: "陕西历史博物馆",
          description: "陕西历史博物馆是中国第一座大型现代化国家级博物馆，被誉为'华夏宝库'，馆藏文物171万余件，上起远古人类初始阶段，下至1840年前。",
      ...
      const collections = [
        {
          ...
          title: "故宫·龙纹文创设计征集",
          description: "以故宫馆藏龙纹瓷器为灵感，征集融合传统龙纹元素的现代文创产品设计方案。包括但不限于：生活用品、文具、服饰配件、数字艺术等品类。",
          artifactName: "清代龙纹青花瓷",
          artifactDescription: "清代官窑精品，以青花工艺绘制五爪龙纹，造型端庄，纹饰精美，是皇家用瓷的典范。",
          images: JSON.stringify(["https://images.unsplash.com/photo-1547981609-4b6bfe67ca0b?w=800&h=600&fit=crop"]),
          requirements: "1. 作品需体现故宫龙纹的文化内涵；2. 设计需具备商业可行性；3. 提交效果图及设计说明",
          prize: "一等奖：50000元，二等奖：20000元，三等奖：10000元",
          prizeAmount: 50000,
          deadline: futureDate,
          status: "active",
        },
        {
          ...
          title: "国博·清明上河图文创开发",
          description: "围绕《清明上河图》这一传世名作，征集创意文创产品设计。希望设计师从画作中提取元素，转化为现代生活用品，让历史名画走入千家万户。",
          artifactName: "清明上河图",
          artifactDescription: "北宋张择端所绘，以精致的笔墨描绘了北宋都城汴京的繁华景象，是中国十大传世名画之一。",
          images: JSON.stringify(["https://images.unsplash.com/photo-1518998053901-5348d3961a04?w=800&h=600&fit=crop"]),
          requirements: "1. 设计元素需来源于画作；2. 产品需具备实用性；3. 提交完整的产品设计方案",
          prize: "优秀奖：30000元，入围奖：5000元",
          prizeAmount: 30000,
          deadline: futureDate,
          status: "active",
        },
        {
          museumId: museumIds[1],
          title: "后母戊鼎·青铜文化创意设计",
          description: "以国宝后母戊鼎为核心，征集体现商代青铜文化的现代创意设计。欢迎各类设计形式，包括产品设计、平面设计、数字艺术等。",
          artifactName: "后母戊鼎",
          artifactDescription: "商代晚期青铜器，重832.84千克，是迄今世界上出土最大、最重的青铜礼器，享有'镇国之宝'美誉。",
          images: JSON.stringify(["https://images.unsplash.com/photo-1518998053901-5348d3961a04?w=800&h=600&fit=crop"]),
          requirements: "1. 设计需体现青铜文化精髓；2. 创意独特，具有市场潜力；3. 提供详细的设计说明",
          prize: "金奖：80000元，银奖：40000元，铜奖：20000元",
          prizeAmount: 80000,
          deadline: futureDate,
          status: "active",
        },
        {
          ...
          title: "苏博·良渚玉文化创意征集",
          description: "以苏州博物馆馆藏良渚文化玉器为灵感，征集融合玉文化元素的现代设计作品。良渚玉器代表了中华文明的曙光，期待设计师赋予其新的生命。",
          artifactName: "良渚玉琮",
          artifactDescription: "良渚文化代表性器物，外方内圆，以精湛的琢玉技艺刻画神人兽面纹，是中华文明起源的重要见证。",
          images: JSON.stringify(["https://images.unsplash.com/photo-1565967511849-76a60a516170?w=800&h=600&fit=crop"]),
          requirements: "1. 设计需体现玉文化的精神内涵；2. 结合现代审美；3. 提交设计稿及创意说明",
          prize: "特等奖：60000元，优秀奖：20000元",
          prizeAmount: 60000,
          deadline: futureDate,
          status: "active",
        },
        {
          museumId: museumIds[3],
          title: "苏州园林·四季美学文创设计",
          description: "以苏州古典园林的四季景致为主题，征集体现江南园林美学的文创产品设计。春花秋月、夏荷冬雪，每一季都有独特的美丽等待设计师去发现。",
          artifactName: "苏州拙政园",
          artifactDescription: "苏州最大的古典园林，始建于明代，以水为主，疏朗典雅，是中国四大名园之一，被列入世界文化遗产。",
          images: JSON.stringify(["https://images.unsplash.com/photo-1565967511849-76a60a516170?w=800&h=600&fit=crop"]),
          requirements: "1. 设计需体现园林四季特色；2. 产品具备实用价值；3. 提交完整设计方案",
          prize: "一等奖：40000元，二等奖：15000元，三等奖：8000元",
          prizeAmount: 40000,
          deadline: futureDate,
          status: "active",
        },
        {
          ...
          title: "陕历博·唐三彩文创设计大赛",
          description: "以陕西历史博物馆馆藏唐三彩为主题，征集体现盛唐文化的创意设计。唐三彩色彩斑斓、造型生动，是盛唐文化自信的最好体现。",
          artifactName: "唐三彩骆驼载乐俑",
          artifactDescription: "唐代陶器珍品，以黄、绿、白三色为主，塑造了一峰载有乐队的骆驼，生动展现了盛唐丝绸之路的繁荣景象。",
          images: JSON.stringify(["https://images.unsplash.com/photo-1564399580075-5dfe19c205f1?w=800&h=600&fit=crop"]),
          requirements: "1. 设计需体现唐三彩的艺术特色；2. 融合现代设计语言；3. 提交效果图及说明书",
          prize: "大奖：100000元，金奖：50000元，银奖：20000元",
          prizeAmount: 100000,
          deadline: futureDate,
          status: "active",
        },
      ...
      const designers = [
        {
          ...
          type: "individual",
          skills: JSON.stringify(["产品设计", "文创设计", "品牌设计"]),
        ...
        },
        {
          ...
          type: "individual",
          skills: JSON.stringify(["插画设计", "平面设计", "数字艺术"]),
        ...
        },
        {
          ...
          type: "individual",
          skills: JSON.stringify(["工业设计", "文创产品", "包装设计"]),
        ...
        },
        {
          ...
          type: "individual",
          skills: JSON.stringify(["珠宝设计", "配饰设计", "东方美学"]),
        ...
        },
        {
          ...
          type: "individual",
          skills: JSON.stringify(["生活美学", "香氛设计", "茶道文化"]),
      ...
      for (const d of designers) {
        ...
        if (existing.length > 0) {
          ...
          log(`  ✓ 设计师已存在: ${d.displayName} (id=${existing[0].id})`);
        } else {
          const [result] = await connection.execute(
          ...
          log(`  ✓ 创建设计师: ${d.displayName} (id=${result.insertId})`);
        }
      }
      ...
      const worksData = [
        {
          ...
          title: "龙腾盛世·茶具套装",
          description: "以故宫馆藏清代龙纹瓷器为灵感，设计一套融合传统龙纹元素的现代茶具。采用景德镇白瓷为基底，以青花工艺绘制简化龙纹，既保留传统韵味，又符合现代审美。套装包含茶壶、茶杯（6只）、茶盘，适合家用及商务礼品。",
